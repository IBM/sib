...
cdef class CSIBOptimizerIntDouble:

    cdef SIBOptimizer[int64_t, double]* c_sib_optimizer  # hold a pointer to the C++ instance which we're wrapping

    def __cinit__(self, int32_t n_clusters, int32_t n_features):
        self.c_sib_optimizer = new SIBOptimizer[int64_t, double](n_clusters, n_features)
...
# Python extension type.
cdef class CSIBOptimizerIntFloat:

    cdef SIBOptimizer[int64_t, float]* c_sib_optimizer  # hold a pointer to the C++ instance which we're wrapping

    def __cinit__(self, int32_t n_clusters, int32_t n_features):
        self.c_sib_optimizer = new SIBOptimizer[int64_t, float](n_clusters, n_features)
    ...
    def init_centroids(self, int32_t n_samples, const int32_t[::1] xy_indices,
                       ...
                       const int64_t[::1] x_sum, int32_t[::1] labels,
                       int32_t[::1] t_size, int64_t[::1] t_sum,
                       float[::1] t_log_sum, int64_t[:,::1] t_centroid):
        self.c_sib_optimizer.init_centroids(n_samples,
                                            &xy_indices[0] if xy_indices is not None else NULL,
                                            &xy_indptr[0] if xy_indptr is not None else NULL,
                                            &xy_data[0], &x_sum[0], &labels[0],
    ...
    def optimize(self, int32_t n_samples, const int32_t[::1] xy_indices,
                 ...
                 int32_t[::1] t_size, int64_t[::1] t_sum,
                 float[::1] t_log_sum, int64_t[:,::1] t_centroid,
                 int32_t[::1] labels, float ity):
        cdef float ht = 0
        cdef float change_rate = 0
        self.c_sib_optimizer.iterate(True, n_samples,
                                     &xy_indices[0] if xy_indices is not None else NULL,
                                     &xy_indptr[0] if xy_indptr is not None else NULL,
                                     &xy_data[0], xy_sum, &x_sum[0],
    ...
    def infer(self, int32_t n_samples, const int32_t[::1] xy_indices,
              ...
              int32_t[::1] t_size, int64_t[::1] t_sum,
              float[::1] t_log_sum, int64_t[:,::1] t_centroid,
              int32_t[::1] labels, float[:,::1] costs):
...
# Python extension type.
cdef class CSIBOptimizerDouble:

    cdef SIBOptimizer[double, double]* c_sib_optimizer  # hold a pointer to the C++ instance which we're wrapping

    def __cinit__(self, int32_t n_clusters, int32_t n_features):
        self.c_sib_optimizer = new SIBOptimizer[double, double](n_clusters, n_features)
...
cdef class CSIBOptimizerFloat:
    ...
    cdef SIBOptimizer[float, float]* c_sib_optimizer  # hold a pointer to the C++ instance which we're wrapping

    def __cinit__(self, int32_t n_clusters, int32_t n_features):
        self.c_sib_optimizer = new SIBOptimizer[float, float](n_clusters, n_features)

    def __dealloc__(self):
        del self.c_sib_optimizer

    def init_centroids(self, int32_t n_samples, const int32_t[::1] xy_indices,
                       const int32_t[::1] xy_indptr, const float[::1] xy_data,
                       const float[::1] x_sum, int32_t[::1] labels,
                       int32_t[::1] t_size, float[::1] t_sum,
                       float[::1] t_log_sum, float[:,::1] t_centroid):
        self.c_sib_optimizer.init_centroids(n_samples,
                                            &xy_indices[0] if xy_indices is not None else NULL,
                                            &xy_indptr[0] if xy_indptr is not None else NULL,
                                            &xy_data[0], &x_sum[0], &labels[0],
    ...
    def optimize(self, int32_t n_samples, const int32_t[::1] xy_indices,
                 const int32_t[::1] xy_indptr, const float[::1] xy_data,
                 float xy_sum, const float[::1] x_sum,
                 int32_t[::1] x_permutation,
                 int32_t[::1] t_size, float[::1] t_sum,
                 float[::1] t_log_sum, float[:,::1] t_centroid,
                 int32_t[::1] labels, float ity):
